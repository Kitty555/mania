...
function fnCertifyCheck(t) {
    ...
    if(t === 'mhpp') {
        ...
        var frmCertify = $('#frmCertify');
        if(mobileA.val().isEmpty() === true) {
            alert("ì¸ì¦ë°›ìœ¼ì‹¤ íœ´ëŒ€í° ë²ˆí˜¸ë¥¼ ìž…ë ¥í•´ì£¼ì„¸ìš”.");
            mobileA.focus();
            return;
        }
        if(mobileB.val().isEmpty() === true) {
            alert("ì¸ì¦ë°›ìœ¼ì‹¤ íœ´ëŒ€í° ë²ˆí˜¸ë¥¼ ìž…ë ¥í•´ì£¼ì„¸ìš”.");
            mobileB.focus();
            return;
        }
        if(mobileC.val().isEmpty() === true) {
            alert("ì¸ì¦ë°›ìœ¼ì‹¤ íœ´ëŒ€í° ë²ˆí˜¸ë¥¼ ìž…ë ¥í•´ì£¼ì„¸ìš”.");
            mobileC.focus();
            return;
        }
        frmCertify.attr({
            action : '/certify/payment/user_certify_number.html',
            onsubmit : ''
        }).submit();
    } else if(t === 'hpp') {
        fnOpenerCheck();
        // 2018-01-31 ì˜¤í›„ 2:51 ìµœì§„ìš±
        // $('#ini_hpp').attr('action', '/certify/ini_hpp_certify/user_certify_form.html').submit();
        $('#ini_hpp').attr('action', '/certify/ini_hpp_certify/user_certify_form_v2_request.php').submit();
    } else if(t === 'logintalk') {
        fnOpenerCheck();
        if( $('#bTalkCheck').val() == true){
            $('#ini_hpp').attr('action', '/certify/logintalk/logintalk_areement.html').submit();
        }else{
            $('#ini_hpp').attr('action', '/certify/logintalk/logintalk_request.php').submit();
        }
    } else if(t === 'pub') {
        fnOpenerCheck();
        _window.open('pub_certify', '', 600, 600);
        $('#ini_pub').attr({
            'target' : 'pub_certify',
            'action' : '/certify/ini_pubauth/pub_auth_request.php'
        }).submit();
        ...
        fnOpenerCheck();
        var frm = $('#emailAuthForm');
        var params = frm.find('[name="certify_lc"]').val();
        ajaxRequest({
            url : '/certify/email/pay_send.html',
            dataType : 'text',
            type : 'POST',
            data : 'certify_lc=' + params,
            success : function(res) {
                if(res == 'OK') {
                    frm.attr('action', '/certify/payment/email_waiting.html').submit();
                }
                else {
                    alert(res);
                }
            ...
            error : function() {
                alert('ì ‘ì†ì´ ì›í•  í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤. ìž ì‹œ í›„ ì´ìš©í•´ ì£¼ì„¸ìš”.');
            }
        });
    }
}
